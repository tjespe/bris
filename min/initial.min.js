...
{};a.domain=location.host;a.width=h.innerWidth;a.height=h.innerHeight;angular.element(h).bind("resize",function(){a.width=h.innerWidth;a.height=h.innerHeight});a.textData={};a.availableLangs=["en","no","es"];a.lang=navigator.language;window.location.href.includes("lang=")&&(a.lang=window.location.href.replace(/.+lang=/,""));a.lang="nb"==a.lang||"nn"==a.lang?"no":a.lang;a.lang=-1<a.availableLangs.indexOf(a.lang)?a.lang:"en";f.get("src/js/objects/"+a.lang+"-text.json",{lifetime:864E6}).then(function(m){a.textData=
...
window.innerWidth+"px");k||e.css("background-position-y","-"+Math.floor(.15*b.pageYOffset)+"px")};angular.element(c).on("resize",d);angular.element(c).on("scroll",d);h.$on("$destroy",function(){angular.element(c).off("resize",d);angular.element(c).off("scroll",d)})}}]);app.value("version",117);