...
window.innerWidth+"px");k||e.css("background-position-y","-"+Math.floor(.15*b.pageYOffset)+"px")};angular.element(c).on("resize",d);angular.element(c).on("scroll",d);h.$on("$destroy",function(){angular.element(c).off("resize",d);angular.element(c).off("scroll",d)})}}]);app.value("version",134);