...
if($norsk){
  $text = json_decode(file_get_contents("src/js/objects/no-text.json"));
} else {
  $text = json_decode(file_get_contents("src/js/objects/en-text.json"));
}
$css = file_get_contents('min/main.min.css');
...
?>
<html ng-app="app" ng-controller="masterCtrl as master" manifest="/src/manifest.php">
...
<body ontouchstart para-back="/src/img/background.jpg" style="background-image:url(/src/img/background.jpg)">
    ...
    <div class="content">
      <div class="nav-menu">
        <ul>
          ...
          <li ng-class="{'active':master.ifSearch()}" ng-if="master.online"><a href="/search"><span class="glyphicon glyphicon-search"></span><span class="nav-menu-text"> <?= $text->{'search'} ?></span></a></li>
  ...
  <div ng-if="master.lazyModulesLoaded" ng-include="'/src/html/cookie-box.html'" ng-controller="cookieCtrl as c"></div>
  ...
  <script src="/min/initial.min.js"></script>
