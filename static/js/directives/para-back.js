app.directive("paraBack", ['$window', function ($window) {
  return function(scope, element, attrs) {
    ...
    image.onload = function () {
      //max = image.height - scope.view.height;
      //var xOffset = -(image.width/2-element[0].offsetWidth/2);
      if (!scope.view.ifMobile && scope.view.width>600) element.css("background-size", scope.view.width+'px');
      else element.css("background-size", scope.view.height*1.5+'px');
      //element.css("background-position-x", xOffset+'px');
    }

    var scrollHandler = function () {
      if (!scope.view.ifMobile && scope.view.width>600) element.css("background-size", scope.view.width+'px');
      var offset = Math.floor(this.pageYOffset*0.15);
      if (!scope.view.ifMobile) element.css('background-position-y', '-'+offset+'px');
    };
